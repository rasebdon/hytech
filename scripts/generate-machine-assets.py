#!/usr/bin/env python3
"""
Generates the electric machines' block textures: a shared casing plus, per machine, an idle and
a `Processing`-lit front.
...
# A colder, lighter steel than the burner's iron, so it doesn't read as a firebox with a new front.
...
def crusher_front(active: bool) -> Rows:
    ...
    for x in range(left + 1, right):
        # Alternate so the two jaws interlock rather than meeting flat.
...
def smelter_front(active: bool) -> Rows:
    ...
    for index, y in enumerate(bars):
        ...
        # Returns on alternating sides, so the bars read as one winding, not three stripes.
