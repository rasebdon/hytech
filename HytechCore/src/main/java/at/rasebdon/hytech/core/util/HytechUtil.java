...
public class HytechUtil {
    ...
    // WorldChunk.getBlockType is deprecated with no replacement offered.
    ...
    /// Item id of the block at `pos`, or null for air, an unloaded chunk, or a block with no item form.
    @Nullable
    ...
    /// Resolves a block entity to its column and position, or null when either has gone. A block
    /// entity lives on a 32-cube `ChunkSection`, so the section-local y decoded from its index has
    /// to be lifted by the section's own y before `WorldChunk` will accept it.
    @Nullable
    ...
    /// Same source vanilla's `Condition` interaction reads for its `Crouching` key.
    ...
    // getRotationIndex is deprecated for removal, but BlockAccessor.getRotation is already gone
    // and nothing else answers this; revisit when the method actually goes.
    ...
    public static BlockTransform getBlockTransform(@Nonnull Ref<ChunkStore> blockRef, @Nonnull Store<ChunkStore> store) {
        ...
        // RotationTuple.get is an unguarded array access; a bad index falls back to no rotation
    ...
    /// `localPos` is the space `WorldChunk`'s own block accessors take: x/z local to the column,
    /// y absolute — not a world position, and not what `BlockStateInfo.fillWorldPos` produces.
    ///
