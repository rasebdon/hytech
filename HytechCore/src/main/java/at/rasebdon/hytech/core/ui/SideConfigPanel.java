...
public final class SideConfigPanel {
    ...
    /// How many resource tabs the document declares (`#Res0`..`#Res4` in MachinePage.ui).
    ///
    /// This is the markup's capacity, not the number of resources: a sixth registered resource
    /// type would not get a tab until the document grows one, so keep the two in step.
    ...
    void render(@Nonnull MachineView view, @Nonnull List<LogisticResourceType> present) {
        ...
        for (int tab = 0; tab < TABS; tab++) {
            ...
            // The colour comes from the resource itself, not from a table here. This used to be
            // the one place in core/ that named energy, items, fluid, gas and heat, which meant a
            // mod could add a resource type but not give it a colour.
