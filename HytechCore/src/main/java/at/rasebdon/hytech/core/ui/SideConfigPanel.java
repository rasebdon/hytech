...
/// Per-resource side configuration, as a panel beside the machine rather than a page over it — so
/// the machine stays visible while its faces are being configured.
///
/// The six faces lay out as a plus; a cell's position says which side, its colour says the mode,
/// and its tooltip spells both out. Sides are configured per resource, with one tab per resource
/// the block actually has, since a machine can carry several containers.
public final class SideConfigPanel {
    ...
    /// Markup capacity for resource tabs (`#Res0`..`#Res4` in MachinePage.ui), not the live count.
    ...
    /// Colours match the wrench's own overlay textures, so the panel needs no legend of its own.
    ...
    /// Index into the *live* present list, so a block losing a container mid-view falls back
    /// to a different tab instead of throwing.
    private int editing;
    ...
    void bind(@Nonnull UIEventBuilder events) {
        ...
        // Bound for every tab the document declares; visibility is decided per-render, not here.
    ...
    void render(@Nonnull MachineView view, @Nonnull List<LogisticResourceType> present) {
        ...
        for (int tab = 0; tab < TABS; tab++) {
            ...
            // Colour comes from the resource itself, so a mod-added resource type gets one for free.
            String colour = type.accent();
    ...
    private void renderPush(@Nonnull MachineView view, @Nonnull LogisticResourceType resource) {
        ...
        // A pipe carries no block component and cannot push on its own.
    ...
    private void renderFace(@Nonnull MachineView view,
        ...
        boolean locked = component == null || component.isFaceLocked(local);
    ...
    /// Converts a world face to the block's own local face, honouring its placed rotation, so a
    /// rotated machine still configures the right side.
    @Nonnull
